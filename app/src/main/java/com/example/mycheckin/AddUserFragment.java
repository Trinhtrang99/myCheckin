...
public class AddUserFragment extends BaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding.btnadd.setOnClickListener(view -> {
            ...
            auth.createUserWithEmailAndPassword(email, "123456789").addOnCompleteListener(requireActivity(), new OnCompleteListener<AuthResult>() {
                @Override
                public void onComplete(@NonNull Task<AuthResult> task) {
                    if (!task.isSuccessful()) {
                        Toast.makeText(requireActivity(), "Create Account success", Toast.LENGTH_SHORT).show();
